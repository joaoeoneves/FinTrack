...
class DashboardViewModelTest {
    ...
    @Test
    fun initialState_isLoading_beforeCollectionStarts() = runTest(testDispatcher) {
    ...
    @Test
    fun emptyRepository_producesContentWithZeroTotalsAndAllCategoriesPresent() = runTest(testDispatcher) {
        val repo = FakeExpenseRepository()
        val viewModel = DashboardViewModel(repo)
    ...
    @Test
    fun defaultTimeRange_isOneMonth_andFiltersOutExpensesOutsideIt() = runTest(testDispatcher) {
        val insideOneMonth = expense(
            id = "inside",
            amountCents = 1_000L,
            category = ExpenseCategory.SHOPPING,
            date = now.minus(3, ChronoUnit.DAYS)
        )
        val outsideOneMonth = expense(
            id = "outside",
            amountCents = 5_000L,
            category = ExpenseCategory.SHOPPING,
    ...
    @Test
    fun categoryTotals_sumCorrectlyPerCategory_acrossAllFourCategories() = runTest(testDispatcher) {
        val shopping1 = expense(id = "s1", amountCents = 1_000L, category = ExpenseCategory.SHOPPING, date = now.minus(1, ChronoUnit.DAYS))
        val shopping2 = expense(id = "s2", amountCents = 500L, category = ExpenseCategory.SHOPPING, date = now.minus(2, ChronoUnit.DAYS))
        val transfer = expense(id = "t1", amountCents = 2_000L, category = ExpenseCategory.TRANSFER, date = now.minus(3, ChronoUnit.DAYS))
        val investment = expense(id = "i1", amountCents = 3_000L, category = ExpenseCategory.INVESTMENTS, date = now.minus(4, ChronoUnit.DAYS))
        val recurring = expense(id = "r1", amountCents = 4_000L, category = ExpenseCategory.RECURRING, date = now.minus(5, ChronoUnit.DAYS))
    ...
    @Test
    fun recentExpenses_returnsTop5ByDateDescending_whenMoreThan5InRange() = runTest(testDispatcher) {
    ...
    @Test
    fun onTimeRangeSelected_switchesFilter_andReQueriesRepository() = runTest(testDispatcher) {
        val withinWeek = expense(id = "within-week", amountCents = 1_000L, category = ExpenseCategory.SHOPPING, date = now.minus(2, ChronoUnit.DAYS))
        val withinMonthNotWeek = expense(id = "within-month", amountCents = 2_000L, category = ExpenseCategory.SHOPPING, date = now.minus(20, ChronoUnit.DAYS))
    ...
    @Test
    fun onTimeRangeSelected_toRangeWithNoExpenses_producesEmptyContent_notBrokenState() = runTest(testDispatcher) {
        val onlyOldExpense = expense(id = "old", amountCents = 1_000L, category = ExpenseCategory.SHOPPING, date = now.minus(200, ChronoUnit.DAYS))
